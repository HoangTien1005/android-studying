...
public class FragmentDetail extends Fragment implements FragmentCallbacks, View.OnClickListener {
    ...
    public void onMsgFromMainToFragment(int pos) {
        currentPos = pos;
        txtId.setText(Member.MEMBERS.get(pos).id);
        txtName.setText(Member.MEMBERS.get(pos).name);
        txtClassroom.setText("Lớp: " +  Member.MEMBERS.get(pos).classroom);
        txtScore.setText("Điểm trung bình: " + Member.MEMBERS.get(pos).score);
    }
